...
class ProjectsController < ApplicationController
  ...
  def index
    index! do
      ...
      @expiring = Project.includes(:user, :category).visible.not_home_page.not_expired.order('expires_at, created_at DESC').limit(3).all
      @recent = Project.includes(:user, :category).visible.not_home_page.not_expired.where("projects.user_id <> 7329").order('created_at DESC').limit(3).all
